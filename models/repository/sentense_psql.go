...
func (_ SentenseRepository) GetAll() ([]Sentense, error) {
	db := db.ConnectDB()
	var s []Sentense
	if err := db.Table("sentenses").Select("sentense_id, title, sentense, user_id").Scan(&s).Error; err != nil {
		return nil, err
	}
	return s, nil
}
...
func (_ SentenseRepository) GetAllByUser(sub string) ([]Sentense, error) {
	db := db.ConnectDB()
	var s []Sentense
	if err := db.Table("sentenses").Where("user_id = ?", sub).Scan(&s).Error; err != nil {
		return nil, err
	}
	return s, nil
}
...
func (_ SentenseRepository) GetBySentenseId(sentense_id string) (models.Sentense, error) {
	...
	if err := db.Table("sentenses").Where("sentense_id = ?", sentense_id).Scan(&s).Error; err != nil {
		return s, err
	}
	return s, nil
}
...
func (_ SentenseRepository) UpdateBySentenseId(sentense_id string, c *gin.Context) (models.Sentense, error) {
	db := db.ConnectDB()
	var s models.Sentense
	if err := db.Table("sentenses").Where("sentense_id = ?", sentense_id).First(&s).Error; err != nil {
		return s, err
	}
	if err := c.BindJSON(&s); err != nil {
		return s, err
...
func (_ SentenseRepository) DeleteBySentenseId(sentense_id string) error {
	...
	if err := db.Table("sentenses").Where("sentense_id = ?", sentense_id).Delete(&s).Error; err != nil {
		return err
	}
